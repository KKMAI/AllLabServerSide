...
class BlogDeleteView(LoginRequiredMixin, PermissionRequiredMixin, views.View):
    ...
    def get(self, request: HttpRequest, pk):
        ...
        if not is_my_blog(request.user, blog.author):
            # ถึงมี permission แต่ถ้าไม่ใช่เจ้าของก็ไม่สามารถลบได้
            # not "Admin" in request.user.groups.all().values_list("name", flat=True)
            if not ("Admin" in request.user.groups.all().values_list("name", flat=True)):
